...
export class CommonService {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      data[i].cortes.forEach((ct) => {
        ...
        const n: ComparativoRend = {
          ...
          peso: peso,
          piezas: piezas,
          pesoCuartos: data[i].pesoCuartos
        };
        comp.push(n);
      });
    }
  ...
  private agruparPorQamark(data: ComparativoRend[]): ComparativoRend[] {  
    ...
    tipos.forEach((t) => {
      ...
      marks.forEach((m) => {        
        if (m != undefined && !m.includes('MANTA') && !m.includes('TRIMM')) {
          ...
          if (e.length > 0) {
            ...
            let peso = e.reduce((acc, item) => acc + item.peso, 0);
            let piezas = e.reduce((acc, item) => acc + item.piezas, 0); 
            let c = e.length;
            let n: ComparativoRend = {
              pesoPromedio: peso / piezas,
              rendimiento: peso / e[0].pesoCuartos,
              qamark: m,
              tipo: t,
              peso: peso,
              piezas: piezas,
  ...
  private setDataToDisplay(tipos: string[], qmarks: string[], data: ComparativoRend[]): ComparativoRend[] {
    ...
    tipos.forEach(t => {
      qmarks.forEach(q => {
        if(q != undefined && !q.includes('MANTA') && !q.includes('TRIMM')) {
